...
(function () {
  ...
  const API_BASE = '(location.hostname==='127.0.0.1'||location.hostname==='localhost'?'http://127.0.0.1:9090':'https://gestioncommandesapi.agreeablepebble-e135b62f.westeurope.azurecontainerapps.io')';
  ...
  function isApiUrlLike(raw) {
    ...
    if (typeof raw === 'string') {
      if (/^//api///i.test(raw)) return true;                              // /api/...
      if (/^https?:////[^/]+//api///i.test(raw)) return true;              // http(s)://*/api/...
      return false;
    }
    try {
      const s = String(raw);
  ...
  function mapToApi(url) {
    ...
    try {
      ...
      return API_BASE.replace(///+$/, '') + path + (u.search || '') + (u.hash || '');
    } catch {
      // Fallback minimal
      return API_BASE.replace(///+$/, '') + normalizeApiPath(String(url || ''));
    }
  }
  ...
  if (origFetch) {
    function patchedFetch(input, init) {
      try {
        ...
        if (isApiUrlLike(urlStr) && !wantsHtml) {
