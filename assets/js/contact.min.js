          ...
          try {
            if (m.extraData)
              for (var j in m.extraData)
                m.extraData.hasOwnProperty(j) &&
                  g.push(
                    a.isPlainObject(m.extraData[j]) &&
                      m.extraData[j].hasOwnProperty("name") &&
                      m.extraData[j].hasOwnProperty("value")
                      ? a(
                          '<input type="hidden" name="' +
                            m.extraData[j].name +
                            '">',
                        )
                          .val(m.extraData[j].value)
                          .appendTo(x)[0]
                      : a('<input type="hidden" name="' + j + '">')
                          .val(m.extraData[j])
                          .appendTo(x)[0],
                  );
      ...
      for (h = 0, m = g.length; m > h; h++)
        if (((l = g[h]), (j = l.name), j && !l.disabled))
          if (b && f.clk && "image" == l.type)
            f.clk == l &&
              (d.push({ name: j, value: a(l).val(), type: l.type }),
              d.push(
                { name: j + ".x", value: f.clk_x },
                { name: j + ".y", value: f.clk_y },
              ));
        ...
        d.ajaxSubmit({
          ...
          success: function (a, c, d, f) {
            if ("ok" === a.status) {
              ...
              f[0].reset();
            } else for (var h in a.messages) b(a.messages[h], "danger");
            e.removeAttr("disabled"),
              e.find("i").removeClass().addClass("fa fa-envelope");
          },
          error: function (a) {
