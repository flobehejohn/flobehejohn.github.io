...
${urlShim}
    ...
    function measureLatencyLoop(root) {
      ...
      function buildProbes(base){
        ...
        const candidates = [PROBE_PATH, '/swagger', '/swagger/index.html', '/'];
        const uniq = Array.from(new Set(candidates.map(p => b + (p.startsWith('/') ? p : ('/' + p)))));
        return [
          ...uniq.filter(u => /swagger|index\.html|\/$/.test(u)).map(u => ({ url:u, method:'GET' })),
          ...uniq.map(u => ({ url:u, method:'HEAD' }))
        ];
      }
  ...
  iframeEl.setAttribute("src", `${BASE}iframe_local.html`);
} else {
